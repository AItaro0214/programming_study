  ...
  return {
    ...
    sections: [
      { t: 'ひとつなぎの例文 — 6つの手順', b: [
        ...
        H('コメントの読み方'),
        P('コードの中の単語は、大きく「言語が決めているもの」と「自分で決めた名前」に分かれます。各行のコメントでは、どちらなのかを必ず書いています。'),
        GLOSS([
          ['予約語', '言語が最初から決めている単語。変えられない。if / for / class / def / func など'],
          ['型名', 'データの種類の名前。String・int は言語が用意したもの、User は自分で作ったもの'],
          ['変数名', '自分で決めた、値を入れる箱の名前。users・count など。好きな名前にできる'],
          ['関数名', '自分で決めた、処理につけた名前。isAdult など'],
          ['引数名', '関数が受け取る値につけた名前。u・user など。その関数の中だけで使う'],
          ['項目名', '型の中に持つデータの名前。name・age など。フィールドとも呼ぶ'],
          ['呼び出し', 'すでにある処理を使うこと。うしろに ( ) が付いていたらだいたいこれ'],
          ['メソッド', '「何かに属している処理」。users.filter(...) の filter がこれ']
        ]),
        NOTE('「言語が決めた単語」と「自分で決めた名前」を見分けられると、\nコードは一気に読めるようになる。'),
        H('Java'),
        CODE('JAVA', '// ① 依存を読み込む\n//    java.time … パッケージ名（機能のまとまり）\n//    LocalDate … その中にある型名。これを参照する\nimport java.time.LocalDate;\nimport java.util.List;   // java.util パッケージの List という型を参照\n\npublic class Main {\n\n    // 「User」という名前の型を作る\n    //    record … 予約語。値を入れるだけのクラスを作る\n    //    User   … 自分で決めた型名。以降 User と書けばこの形を指す\n    //    String name … 「String型」の項目を「name」という名前で持つ\n    //    int age     … 「int型」の項目を「age」という名前で持つ\n    record User(String name, int age) {}\n\n    // ③ 「isAdult」という名前の関数を作る\n    //    static  … 予約語。newせずに呼び出せる印\n    //    boolean … 戻り値の型。true か false を返す\n    //    isAdult … 自分で決めた関数名\n    //    User u  … 引数。「User型」の値を「u」という名前で受け取る\n    static boolean isAdult(User u) {\n        return u.age() >= 18;   // u.age() は User の項目 age を取り出す呼び出し\n    }\n\n    // ⑥ プログラムの入口。この名前と形は決まり文句\n    public static void main(String[] args) {\n\n        // ② 「users」という名前の変数を作る\n        //    List<User> … 型。「User型を並べたリスト」という意味\n        //    users      … 自分で決めた変数名\n        //    List.of(…) … リストを作る呼び出し\n        //    new User("Taro", 20) … User型の値を1つ作る（name="Taro", age=20）\n        List<User> users = List.of(\n            new User("Taro", 20),\n            new User("Hana", 15)\n        );\n\n        // ④ 条件で絞る\n        //    long  … 型。件数を入れる整数\n        //    count … 自分で決めた変数名\n        //    stream() / filter() / count() … すべて呼び出し（処理）\n        //    Main::isAdult … 上で作った関数「そのもの」を渡している\n        long count = users.stream().filter(Main::isAdult).count();\n\n        // ⑤ 条件で分ける（if / else は予約語）\n        if (count > 0) {\n            // LocalDate.now() は ① で読み込んだ型が持つ呼び出し\n            System.out.println(LocalDate.now() + " 成人 " + count + " 人");\n        } else {\n            System.out.println("成人はいません");\n        }\n    }\n}'),
        H('JavaScript'),
        CODE('JAVASCRIPT', '// ① 依存を読み込む\n//    "node:os" … モジュール名（どこから持ってくるか）\n//    hostname  … その中にある関数名。{ } で名指しして取り出す\n//    外部パッケージなら import axios from "axios";\nimport { hostname } from "node:os";\n\n// ② 「users」という名前の変数を作る\n//    const … 予約語。再代入できない変数を作る\n//    users … 自分で決めた変数名\n//    [ ]   … 配列。その中の { } はオブジェクト\n//    name / age … 自分で決めたキー名。JavaScriptは型を書かない\nconst users = [\n  { name: "Taro", age: 20 },\n  { name: "Hana", age: 15 },\n];\n\n// ③ 「isAdult」という名前の関数を作る\n//    function … 予約語。関数を定義する合図\n//    isAdult  … 自分で決めた関数名\n//    user     … 引数名。型は書かない\nfunction isAdult(user) {\n  return user.age >= 18;   // user.age はオブジェクトの項目 age を読む\n}\n\nfunction main() {\n  // ④ 条件で絞る\n  //    filter … 配列が持つメソッド。isAdult を1件ずつ呼び、true だけ残す\n  //    length … 件数を返すプロパティ。( ) が付かない点に注意\n  //    count  … 自分で決めた変数名\n  const count = users.filter(isAdult).length;\n\n  // ⑤ 条件で分ける\n  if (count > 0) {\n    // ` ` … テンプレートリテラル。${ } の中が値として埋め込まれる\n    // hostname() … ① で読み込んだ関数の呼び出し\n    console.log(`${hostname()} 成人 ${count} 人`);\n  } else {\n    console.log("成人はいません");\n  }\n}\n\n// ⑥ 実行する。定義しただけでは動かない。呼んで初めて動く\nmain();'),
        H('TypeScript'),
        CODE('TYPESCRIPT', '// ① 依存を読み込む — JavaScriptとまったく同じ書き方\nimport { hostname } from "node:os";\n\n// 「User」という名前の型を作る（TypeScriptだけの機能。実行時には消える）\n//    interface … 予約語。データの形に名前を付ける\n//    User      … 自分で決めた型名\n//    name: string … 「name」という項目が「string型」だという宣言\ninterface User {\n  name: string;\n  age: number;\n}\n\n// ② 「users」という名前の変数を作る\n//    users    … 自分で決めた変数名\n//    : User[] … 型注釈。「User型を並べた配列」という意味\nconst users: User[] = [\n  { name: "Taro", age: 20 },\n  { name: "Hana", age: 15 },\n];\n\n// ③ 「isAdult」という名前の関数を作る\n//    user: User … 引数「user」が「User型」だという宣言\n//    : boolean  … 戻り値が「boolean型」だという宣言\nfunction isAdult(user: User): boolean {\n  return user.age >= 18;\n}\n\n// : void … 「何も返さない」という型\nfunction main(): void {\n  // ④ 条件で絞る\n  //    : number … count が数値型だという宣言（書かなくても推論される）\n  const count: number = users.filter(isAdult).length;\n\n  // ⑤ 条件で分ける\n  if (count > 0) {\n    console.log(`${hostname()} 成人 ${count} 人`);\n  } else {\n    console.log("成人はいません");\n  }\n}\n\n// ⑥ 実行する\nmain();'),
        NOTE('JavaScript と TypeScript の差は「: 型」だけ。\n流れはまったく同じ。')
      ]},
      { t: 'ひとつなぎの例文 — Python / Dart / Swift / Kotlin', b: [
        H('Python'),
        CODE('PYTHON', '# ① 依存を読み込む\n#    datetime … モジュール名（どこから持ってくるか）\n#    date     … その中にある型名。from ... import ... で名指しして参照\nfrom datetime import date\n\n# ② 「users」という名前の変数を作る\n#    users … 自分で決めた変数名。Pythonは型を書かない\n#    [ ]   … リスト\n#    { }   … 辞書。"name" と "age" は自分で決めたキー名（ただの文字列）\nusers = [\n    {"name": "Taro", "age": 20},\n    {"name": "Hana", "age": 15},\n]\n\n# ③ 「is_adult」という名前の関数を作る\n#    def      … 予約語。関数を定義する合図\n#    is_adult … 自分で決めた関数名（小文字とアンダースコアが慣習）\n#    user     … 引数名\n#    :        … この後ろはインデントで中身を表す\ndef is_adult(user):\n    return user["age"] >= 18   # 辞書は ["キー名"] で取り出す\n\ndef main():\n    # ④ 条件で絞る（リスト内包表記）\n    #    u              … その場だけで使う変数名\n    #    for u in users … users から1件ずつ u に入れる\n    #    if is_adult(u) … 条件。true のものだけ集める\n    #    len(...)       … 件数を返す組み込み関数の呼び出し\n    count = len([u for u in users if is_adult(u)])\n\n    # ⑤ 条件で分ける\n    if count > 0:\n        # f"..." … f文字列。{ } の中が値として埋め込まれる\n        # date.today() … ① で読み込んだ型が持つ呼び出し\n        print(f"{date.today()} 成人 {count} 人")\n    else:\n        print("成人はいません")\n\n# ⑥ 実行する\n#    __name__ … 実行中のファイル名が入る変数。直接実行なら "__main__" になる\nif __name__ == "__main__":\n    main()'),
        H('Dart'),
        CODE('DART', '// ① 依存を読み込む — "dart:io" は標準ライブラリのモジュール名\nimport "dart:io";\n\n// 「User」という名前の型を作る\n//    class … 予約語。型を作る合図\n//    User  … 自分で決めた型名\nclass User {\n  final String name;   // 「String型」の項目を「name」で持つ。final=変更不可\n  final int age;       // 「int型」の項目を「age」で持つ\n  User(this.name, this.age);   // コンストラクタ。this.name と書くだけで代入される\n}\n\n// ③ 「isAdult」という名前の関数を作る\n//    bool    … 戻り値の型\n//    isAdult … 自分で決めた関数名\n//    User u  … 「User型」の値を「u」という名前で受け取る\n//    =>      … 中身が1行のとき return を省略できる書き方\nbool isAdult(User u) => u.age >= 18;\n\n// ⑥ ここから実行が始まる。void は「何も返さない」\nvoid main() {\n  // ② 「users」という名前の変数を作る。final は再代入できない\n  final users = [User("Taro", 20), User("Hana", 15)];\n\n  // ④ 条件で絞る\n  //    where  … Dartでの絞り込みメソッド名（filterではない）\n  //    length … 件数を返すプロパティ\n  final count = users.where(isAdult).length;\n\n  // ⑤ 条件で分ける\n  if (count > 0) {\n    // "$count" … 変数の値を文字列に埋め込む\n    // stdout … ① で読み込んだ dart:io が持つオブジェクト\n    stdout.writeln("成人 $count 人");\n  } else {\n    stdout.writeln("成人はいません");\n  }\n}'),
        H('Swift'),
        CODE('SWIFT', '// ① 依存を読み込む — Foundation は標準ライブラリのモジュール名\nimport Foundation\n\n// 「User」という名前の型を作る\n//    struct … 予約語。コピーで渡る型を作る\n//    User   … 自分で決めた型名\nstruct User {\n    let name: String   // 「name」という項目が「String型」。let=変更不可\n    let age: Int       // 「age」という項目が「Int型」\n}\n\n// ③ 「isAdult」という名前の関数を作る\n//    func      … 予約語。関数を定義する合図\n//    isAdult   … 自分で決めた関数名\n//    _ u: User … 引数。「User型」を「u」で受け取る\n//                _ は「呼ぶとき引数名を省く」印\n//    -> Bool   … 戻り値が「Bool型」\nfunc isAdult(_ u: User) -> Bool {\n    return u.age >= 18\n}\n\nfunc main() {\n    // ② 「users」という名前の変数を作る\n    //    User(name:age:) … 型から値を作る。項目名を書いて渡す決まり\n    let users = [\n        User(name: "Taro", age: 20),\n        User(name: "Hana", age: 15)\n    ]\n\n    // ④ 条件で絞る。filter も count も配列が持つ機能\n    let count = users.filter(isAdult).count\n\n    // ⑤ 条件で分ける。Swiftは条件に ( ) が要らない\n    if count > 0 {\n        // \\( ) … この中が値として埋め込まれる\n        print("成人 \\(count) 人")\n    } else {\n        print("成人はいません")\n    }\n}\n\n// ⑥ 実行する\nmain()'),
        H('Kotlin'),
        CODE('KOTLIN', '// ① 依存を読み込む — Javaの標準ライブラリの型をそのまま参照できる\nimport java.time.LocalDate\n\n// 「User」という名前の型を作る\n//    data class … 予約語。値を入れるだけのクラスを1行で作る\n//    User       … 自分で決めた型名\n//    val name: String … 「name」という項目が「String型」。val=変更不可\ndata class User(val name: String, val age: Int)\n\n// ③ 「isAdult」という名前の関数を作る\n//    fun       … 予約語。関数を定義する合図\n//    isAdult   … 自分で決めた関数名\n//    u: User   … 引数「u」が「User型」\n//    : Boolean … 戻り値の型\n//    = ...     … 中身が1行なら = で書ける（return を省略）\nfun isAdult(u: User): Boolean = u.age >= 18\n\n// ⑥ ここから実行が始まる\nfun main() {\n    // ② 「users」という名前の変数を作る。val=変更不可、var=変更可\n    val users = listOf(User("Taro", 20), User("Hana", 15))\n\n    // ④ 条件で絞る\n    //    ::isAdult … 上で作った関数「そのもの」を指す書き方\n    //    size      … 件数を返すプロパティ\n    val count = users.filter(::isAdult).size\n\n    // ⑤ 条件で分ける\n    if (count > 0) {\n        // "$変数" で埋め込む。式を入れるときは ${ }\n        println("${LocalDate.now()} 成人 $count 人")\n    } else {\n        println("成人はいません")\n    }\n}'),
      ...
      { t: 'ひとつなぎの例文 — C / C++', b: [
        ...
        H('C'),
        CODE('C', '// ① 依存を読み込む\n//    #include … コンパイル前の指示。ファイルを丸ごと貼り付ける\n//    < >      … システムの標準ライブラリから探す印\n#include <stdio.h>     // printf を使うため\n#include <stdbool.h>   // bool / true / false を使うため\n\n// 「User」という名前の型を作る（Cにクラスはないので構造体を使う）\n//    struct  … 予約語。項目をまとめた型を作る\n//    typedef … 予約語。その型に「User」という名前を付ける\n//    char name[32] … 「name」は「32文字ぶんの文字の配列」\n//    int age       … 「age」は「int型」\ntypedef struct {\n    char name[32];\n    int age;\n} User;\n\n// ③ 「is_adult」という名前の関数を作る\n//    bool     … 戻り値の型\n//    is_adult … 自分で決めた関数名\n//    User u   … 「User型」の値を「u」という名前で受け取る\nbool is_adult(User u) {\n    return u.age >= 18;   // u.age は構造体の項目を読む\n}\n\n// ⑥ プログラムの入口。int は「整数を返す」、void は「引数なし」\nint main(void) {\n    // ② 「users」という名前の変数を作る\n    //    User users[] … 「User型の配列」。名前は users\n    User users[] = {{"Taro", 20}, {"Hana", 15}};\n\n    // 件数も自分で計算する（配列全体のバイト数 ÷ 1件ぶんのバイト数）\n    //    len … 自分で決めた変数名\n    int len = sizeof(users) / sizeof(users[0]);\n\n    // ④ 条件で絞る。Cに filter は無いので for で自分で数える\n    //    count … 自分で決めた変数名。0から数え上げる\n    //    i     … ループ用の変数名（慣習的に i を使う）\n    int count = 0;\n    for (int i = 0; i < len; i++) {\n        if (is_adult(users[i])) {\n            count++;\n        }\n    }\n\n    // ⑤ 条件で分ける\n    if (count > 0) {\n        // %d … 「ここに整数を入れる」印。\\n は改行\n        printf("成人 %d 人\\n", count);\n    } else {\n        printf("成人はいません\\n");\n    }\n    return 0;   // 0 は「正常終了」をOSに伝える値\n}'),
        H('C++'),
        CODE('C++', '// ① 依存を読み込む — 使う機能ごとに1行ずつ書く\n#include <iostream>    // std::cout を使うため\n#include <vector>      // std::vector を使うため\n#include <string>      // std::string を使うため\n#include <algorithm>   // std::count_if を使うため\n\n// 「User」という名前の型を作る\n//    struct … 予約語。項目をまとめた型を作る\n//    std::string name … 「name」は「文字列型」。std:: は標準ライブラリの印\n//    int age          … 「age」は「int型」\nstruct User {\n    std::string name;\n    int age;\n};\n\n// ③ 「isAdult」という名前の関数を作る\n//    bool          … 戻り値の型\n//    isAdult       … 自分で決めた関数名\n//    const User& u … 「User型」を「u」で受け取る\n//                    & はコピーせず参照する印、const は書き換えない約束\nbool isAdult(const User& u) {\n    return u.age >= 18;\n}\n\n// ⑥ プログラムの入口\nint main() {\n    // ② 「users」という名前の変数を作る\n    //    std::vector<User> … 型。「User型を並べた可変長配列」\n    std::vector<User> users = {{"Taro", 20}, {"Hana", 15}};\n\n    // ④ 条件で絞る\n    //    count_if        … 範囲を見て条件に合う件数を返す関数の呼び出し\n    //    begin() / end() … 先頭と末尾を指すもの\n    //    count           … 自分で決めた変数名\n    int count = std::count_if(users.begin(), users.end(), isAdult);\n\n    // ⑤ 条件で分ける\n    if (count > 0) {\n        // << … 右へ流し込む演算子。endl は改行\n        std::cout << "成人 " << count << " 人" << std::endl;\n    } else {\n        std::cout << "成人はいません" << std::endl;\n    }\n    return 0;\n}'),
